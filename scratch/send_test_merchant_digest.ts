...
async function main() {
  ...
  try {
    ...
    console.log(`Building digest for ${year}-${String(month).padStart(2, '0')}ù`);
    ...
    console.log(`Sending PDF (${pdfBuffer.length} bytes) to ${recipient}ù`);
    ...
    if (!ok) {
      throw new Error('Brevo send returned false ù check BREVO_API_KEY / SMTP_FROM');
    }
